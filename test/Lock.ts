import { time, loadFixture } from '@nomicfoundation/hardhat-toolbox-viem/network-helpers.js'
...
describe('Lock', function () {
  ...
  async function deployOneYearLockFixture() {
    ...
    const lock = await hre.viem.deployContract<string>('Lock', [unlockTime], {
      value: lockedAmount
    })
  ...
  describe('Deployment', function () {
    ...
    it('Should fail if the unlockTime is not in the future', async function () {
      ...
      await expect(
        hre.viem.deployContract<string>('Lock', [latestTime], {
          value: 1n
        })
  ...
  describe('Withdrawals', function () {
    ...
    describe('Events', function () {
      it('Should emit an event on withdrawals', async function () {
        ...
        expect((withdrawalEvents[0].args as { amount: bigint }).amount).to.equal(lockedAmount)
      })
    })
  })
})
